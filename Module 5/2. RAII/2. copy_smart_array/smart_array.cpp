...
smart_array& smart_array::operator = (const smart_array& operand) {
	delete[] pArray;
	pArray = new int[operand.logic_size];
	logic_size = operand.logic_size;
	fact_size = operand.logic_size;
	for (int i = 0; i < logic_size; ++i) {
		pArray[i] = operand.pArray[i];
	}
