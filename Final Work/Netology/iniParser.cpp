...
template <typename T>
T iniParser::get_value(std::string key) {
...
template <>
std::string iniParser::get_value(std::string key) {
    if (values.find(key) == values.end()) {
        throw std::runtime_error(not_found_message(key));
    }
    else {
...
template <>
int iniParser::get_value(std::string key) {
    if (values.find(key) == values.end()) {
        throw std::runtime_error(not_found_message(key));
    }
    else {
...
template <>
bool iniParser::get_value(std::string key) {
    if (values.find(key) == values.end()) {
        throw std::runtime_error(not_found_message(key));
    }
    else {
...
std::string iniParser::delete_spaces(std::string in, int ini_line) {
...
std::string iniParser::not_found_message(std::string key) { //вспомогательная функция - формирует сообщение для исключения
